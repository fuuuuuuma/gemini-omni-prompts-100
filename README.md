...
- **未確認**: 「Gemini Omni」というモデル名・仕様は本リポジトリ作成時点（2026年5月）で公式リリース情報を作者側で完全には確認できていません。本コンテンツは「動画編集に特化したマルチモーダルAI／video-to-video モデル」を前提とした一般的な video-to-video プロンプト設計です。実際のモデル挙動と異なる場合は、各自の現場で数値や表現を調整してください。
- **プロンプト設計の基本**: 「**保持すべき要素**」と「**変換すべき要素**」を明示的に分離する記述を強く推奨します。これが video-to-video の精度を決める最大要因です。
- **数値で固定**: `35%寄った` / `0.5倍速` のように数値で意図を固定すると結果がブレません。
- **ネガティブプロンプト**: video-to-video では効果が大きい。意図しない要素は明示的に排除してください。
